...
class Level():
  ...
  def x_collision(self):
    ...
    if player.rect.x - player.speed >= 0 and player.rect.x < len(map[0]) * sprite_size:
      player.rect.x += player.direction.x * player.speed

    if (player.rect.right > screen_width - 200 and self.bg_scroll < (120 * sprite_size) - screen_width) \
      or (player.rect.x < self.camera_pos.x + 220 and self.bg_scroll > abs(player.speed)):
      
      if player.speed > 0 and player.direction.x > 0:
        self.bg_scroll += player.speed
      
      elif player.direction.x < 0 and player.rect.x < self.camera_pos.x + 215:
        self.bg_scroll -= player.speed
